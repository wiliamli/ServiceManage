...
namespace Jwell.Modules.Cache.Redis
{
    public sealed class RedisHashRedis
    {
        ...
        public RedisHashRedis(long db = 0)
        {
            //db默认数量
            if (db >= 0 && db < 16)
            {
        ...
        public void SetHRang(string hashId, ConcurrentDictionary<string, string> hash)
        {
            lock (objLock) //线程安全
            {
                RedisManagement.RedisClient.SetRangeInHash(hashId, hash);
            }
        }
        ...
        public void SetHV(string hashId,string key,string value)
        {
            lock (objLock) //线程安全
            {
                RedisManagement.RedisClient.SetEntryInHash(hashId, key,value);
            }
        }
        ...
        {
            return RedisManagement.RedisClient.GetValueFromHash(hashId, key);
        }
        ...
        {
            return RedisManagement.RedisClient.GetAllEntriesFromHash(hashId);
        }
        ...
        {
            return RedisManagement.RedisClient.GetHashKeys(hashId);
        }
        ...
        {
            return RedisManagement.RedisClient.GetHashValues(hashId);
        }
        ...
        {
            return RedisManagement.RedisClient.RemoveEntryFromHash(hashId, key);
        }
        ...
        public bool ReomveHRange(string hashId, IEnumerable<string> keys)
        {
            ...
            lock (objLock) //线程安全
            {
                ...
                {
                    success = RedisManagement.RedisClient.RemoveEntryFromHash(hashId, key);
                }
            }
            return success;
        }
        ...
        {
            return RedisManagement.RedisClient.HashContainsEntry(hashId, key);
        }
        ...
        {
           return RedisManagement.RedisClient.HLen(hashId) > 0;
        }
    }
}
